...
import { deleteSouscompte, updateSouscompte } from './redux/MySlicer';

const GéstionSousCompte = () => {
  // Récupération des sous-comptes depuis le store Redux
  const souscomptes = useSelector((state) => state.souscompte.souscomptes);
  const dispatch = useDispatch();

  // États locaux pour la gestion de l'édition d'un sous-compte
  const [editId, setEditId] = useState(null);
  const [newNom, setNewNom] = useState('');
  const [newPrenom, setNewPrenom] = useState('');
  const [newPassword, setNewPassword] = useState('');

  // Gestion du mode édition : remplit les champs avec les valeurs existantes
  const handleEdit = (sc) => {
    setEditId(sc.id);
    setNewNom(sc.nom);
  ...
  // Met à jour un sous-compte via Redux
  const handleUpdate = () => {
    dispatch(updateSouscompte({
      id: editId,
      nom: newNom,
      prenom: newPrenom,
      password: newPassword
    }));
    setEditId(null); // Sort du mode édition
  };

  // Supprime un sous-compte après confirmation utilisateur
  const handleDelete = (id) => {
    const confirmDelete = window.confirm("Êtes-vous sûr de vouloir supprimer ce sous-compte?");
    if (confirmDelete) {
      dispatch(deleteSouscompte(id));
                    ...
                    <tbody className="text-gray-700">
                      {souscomptes.map((sc) => (
                        <tr key={sc.id} className="hover:bg-gray-200">
                          {/* Affichage du login ou champ modifiable en mode édition */}
                          ...
                          {/* Affichage ou édition du prénom */}
                          <td className="px-4 py-2">
                            {editId === sc.id
                              ? <input
                                  className="border p-1 rounded"
                          ...
                          {/* Actions : Modifier / Supprimer ou Enregistrer en mode édition */}
...
export default GéstionSousCompte;
